...
class _FunctionToolApprovalKey(str):
    ...
    def __new__(cls, agent: Any, tool_key: str) -> _FunctionToolApprovalKey:
        ...
        value = cls._PREFIX + json.dumps([identity[1], tool_key, agent.name], separators=(",", ":"))
        key = super().__new__(cls, value)
        key._agent_ref = identity[0]
        key.tool_key = tool_key
        key.owner_name = agent.name
    ...
    def _from_serialized(cls, value: str) -> _FunctionToolApprovalKey:
        ...
        # Only Runner's configured graph can resolve the transported owner name.
        # The process-local token distinguishes source owners, never live objects.
        key._agent_ref = None
        key.tool_key = identity[1]
        key.owner_name = identity[2]
...
class RunContextWrapper(Generic[TContext]):
    ...
    def _resolve_function_approval_owners(self, starting_agent: Any) -> None:
        """Bind transported decisions only to unambiguous configured owners."""
        detached = [
            key
        ...
        configured: dict[str, dict[int, Any]] = {}
        for agent in _iter_agent_graph(starting_agent):
            owner = get_public_agent(agent)
            configured.setdefault(owner.name, {})[id(owner)] = owner
        source_owners: dict[str, set[str]] = {}
        for key in self._approvals:
            if isinstance(key, _FunctionToolApprovalKey):
                owner = key.agent
                name = owner.name if owner is not None else key.owner_name
                source_owners.setdefault(name, set()).add(key.owner_token)
        for key in detached:
            owners = configured.get(key.owner_name, {})
            # Keep unresolved records dormant for later runs with another graph.
            # They cannot match owner-bound execution or enter this checkpoint.
            if len(owners) != 1 or len(source_owners[key.owner_name]) != 1:
                continue
            record = self._approvals.pop(key)
            owner = next(iter(owners.values()))
            resolved = _FunctionToolApprovalKey(owner, key.tool_key)
    ...
    def _approval_records_for_key(
        ...
        *,
        ...
        legacy = self._approvals.get(tool_key)
    ...
    def _get_approval_status_for_key(self, approval_key: str, call_id: str) -> bool | None:
        ...
        for record in self._approval_records_for_key(approval_key, call_id=call_id):
    ...
    def get_approval_status(
        ...
        *,
        ...
        for candidate in candidates:
            ...
            records = (
                self._approval_records_for_key(candidate, selected_invocation)
                if selected_invocation is not None
                else (self._approvals[candidate],)
                if candidate in self._approvals
                else ()
            )
